$(document).ready(function() {
    ...
    if ($("#service-history").length) {
        table = new Table('#service-history');
    }

    if ($("#raf-history").length) {
        table = new Table('#raf-history');
    }

    if ($("#sor-history").length) {
        table = new Table('#sor-history');
    }
    ...
    if ($("#realm-table").length) {
        Services.dataTableRealm = new Table('#realm-table', {
			paging: true,
			results: 8,
            pageCount: 6,
			totalResults: Services.realmID.length,
			cache: true
		});
    }
});
...
var Services = {
    ...
    initTypeAhead: function() {
        ...
        Services.typeAhead = new TypeAhead('#realm-type-ahead', {
            minLength: 3,
            resultsLength: 10,
            termMatch: true,
            groupResults: true,
        ...
        $("#realm-type-ahead").bind('keyup change blur',function() {
            if ($(this).val().length != 0) {
                for (var i = 0, length = Services.realmID.length; i < length; i++) {
                    if ($(this).val() === Services.realmName[i]) {
...
filterRealmDataTable: function() {
	...
	_isCharacterEligible: function (target, license, region, character) {
		...
		$.ajax({
			url: Core.staticUrl + '/management/wow/services/is-character-eligible.html',
			data: ({
				service: serviceName,
				l: license,
				r: region,
