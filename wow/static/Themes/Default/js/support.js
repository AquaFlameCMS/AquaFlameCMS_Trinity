...
var Inputs = Class.extend({
	...
	setup: function() {
		...
		if (this.form[0].tagName.toLowerCase() === 'form') {
			...
			if (this.textareaInputs.length && config.placeholder && !this.supportsPlaceholder(false)) {
	...
	/**
	 * Process the form (disable the submit button).
	 *
	 * @param	e The event data.
	 */
	_processForm: function(e) {
		this.submitButton.removeClass('hover').addClass('processing').attr('disabled', 'disabled');
	},
...
var DateTime = Class.extend({
	...
	localize: function() {
		...
		for (var i = 0, len = times.length; i < len; i++) {
			...
			if ($.browser.msie) {
				if (time.hasClass('relative')) {
					time.parent().attr('data-tooltip', datetime).attr('data-tooltip-options', '{"location": "mouse"}');
				} else {
					time.parent().html(datetime);
				}
			} else {
				if (time.hasClass('relative')) {
					time.attr('data-tooltip', datetime).attr('data-tooltip-options', '{"location": "mouse"}');
				} else {
					time.html(datetime);
				}
			}
